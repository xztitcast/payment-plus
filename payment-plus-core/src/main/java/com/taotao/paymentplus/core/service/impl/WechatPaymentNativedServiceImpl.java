...
@Slf4j
@Service
public class WechatPaymentNativedServiceImpl extends WeChatPaymentSupport implements PaymentService {
	
	private WechatGlobalConfiguration configuration;

	public WechatPaymentNativedServiceImpl(WechatGlobalConfiguration configuration) {
		super(configuration.getNatived());
	...
	public Map<String, String> prepay(Param param) {
		...
		if(response !=null && "SUCCESS".equals(response.get("return_code")) && "SUCCESS".equals(response.get("result_code"))) {
			sort.put("code_url", response.get("code_url"));
		}
		return sort;
	}
