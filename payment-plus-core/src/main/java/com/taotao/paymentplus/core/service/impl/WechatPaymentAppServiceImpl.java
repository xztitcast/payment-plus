...
@Slf4j
@Service
public class WechatPaymentAppServiceImpl extends WeChatPaymentSupport implements PaymentService {
	
	private WechatGlobalConfiguration configuration;

	public WechatPaymentAppServiceImpl(WechatGlobalConfiguration configuration) {
		super(configuration.getApp());
	...
	public Map<String, String> prepay(Param param) {
		...
		if(response !=null && "SUCCESS".equals(response.get("return_code")) && "SUCCESS".equals(response.get("result_code"))) {
			...
			sort.put("partnerid", response.get("mch_id"));
			sort.put("prepayid", response.get("prepay_id"));
